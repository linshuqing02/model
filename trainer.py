...
from utils import prepare_labels, get_sample_loader, visit_level, code_level, calculate_sensitivity, \
    calculate_specificity, log_outmemory
...
class EarlyStopper:
    def __init__(self, patience=5, min_delta=0.0001, mode='min'):
        ...
        Args:
            ...
            mode (str): 'min' for minimizing metrics (e.g. loss), 'max' for maximizing (e.g. accuracy)
        """
...
def training(args, data_loader, model, label_tokenizer, optimizer, label_name, log_outmemory_txt_path, device):
    model.train()
    train_loss = 0
    with tqdm(total=len(data_loader), desc="Training", unit="batch") as pbar:
        for batch_idx, data in enumerate(data_loader):
            ...
            if args.model == "Base2_1_3":
                try:
                    out, loss_recon, loss_orth = model(data)
                ...
                #     exit("前向输出中有NaN或Inf值！")
                loss_bce = F.binary_cross_entropy_with_logits(out, label)
                # if torch.isnan(loss).any() or torch.isinf(loss).any():
                #     exit("损失中有NaN或Inf值！")
                loss = loss_bce + args.coef_loss_recon * loss_recon + args.coef_loss_orth * loss_orth
                # if torch.isnan(loss).any() or torch.isinf(loss).any():
                #     exit("损失中有NaN或Inf值！")
            else:
                try:
...
def evaluating(args, data_loader, model, label_tokenizer, label_name, device):
    model.eval()
    val_loss = 0
    y_t_all, y_p_all = [], []
    with torch.no_grad():
...
def testing(args, data_loader, test_epochs, model, label_tokenizer, sample_size, label_name, device):
    results = []
    for epoch in range(test_epochs):
        print(f'\nTesting Epoch {epoch + 1}/{test_epochs}')
        sample_loader = get_sample_loader(data_loader, sample_size)

        _, metrics, code_level_results, visit_level_results, sensitivity, specificity = evaluating(args, sample_loader, model,
                                                                                                   label_tokenizer,
                                                                                                   label_name, device)
