...
def drug_recommendation_mimic3_fn(patient: Patient):
    ...
    for visit in visits:
        ...
        time_seq = [(x[0], 'b') for x in lab_time_groups] + [(x[0], 'a') for x in inj_time_groups]
        time_seq = [ v.__add__((k,)) for k, v in enumerate(time_seq)]
        sorted_list = sorted(time_seq, key=lambda x: (x[0], x[1]))
        sorted_index_2 = [x[2] for x in sorted_list]
        ...
        samples.append(
            {
                ...
                'ing_lab_time_index': sorted_index_2,
    ...
    samples[0]["ing_lab_time_index"] = [samples[0]["ing_lab_time_index"]]
    ...
    for i in range(1, len(samples)):
        ...
        samples[i]["ing_lab_time_index"] = samples[i - 1]["ing_lab_time_index"] + [samples[i]["ing_lab_time_index"]]
