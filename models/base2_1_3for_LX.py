...
class Base2_1_3(BaseModel):
    def __init__(
        ...
        # 添加共享编码器
        self.shared_encoder_2d = nn.Sequential(
            nn.Linear(embedding_dim, embedding_dim),
            nn.ReLU(),
            nn.Linear(embedding_dim, embedding_dim)
        )
        self.shared_encoder_3d = nn.Sequential(
            nn.Linear(embedding_dim, embedding_dim),
            nn.ReLU(),
            nn.Linear(embedding_dim, embedding_dim)
        )
        ...
        # 添加特定编码器
        self.specific_encoder_2d = nn.Sequential(
            nn.Linear(embedding_dim, embedding_dim),
            nn.ReLU(),
            nn.Linear(embedding_dim, embedding_dim)
        )
        self.specific_encoder_3d = nn.Sequential(
            nn.Linear(embedding_dim, embedding_dim),
            nn.ReLU(),
            nn.Linear(embedding_dim, embedding_dim)
        )
    ...
    def calculate_orthogonality_loss(self, shared_2d_list, shared_3d_list, specific_2d_list, specific_3d_list):
        total_loss = 0
        count = len(shared_2d_list)
        
        for shared_2d, shared_3d, specific_2d, specific_3d in zip(
            shared_2d_list, shared_3d_list, specific_2d_list, specific_3d_list
        ):
            # 归一化向量（重要：确保向量是单位向量）
            shared_2d_norm = F.normalize(shared_2d, dim=-1)
            shared_3d_norm = F.normalize(shared_3d, dim=-1)
            specific_2d_norm = F.normalize(specific_2d, dim=-1)
            specific_3d_norm = F.normalize(specific_3d, dim=-1)
            
            # 正交损失：最小化余弦相似度的平方
            # specific 2d与specific 3d正交
            ortho1 = torch.mean(torch.sum(specific_2d_norm * specific_3d_norm, dim=-1) ** 2)
            
            # specific 2d与shared 2d正交  
            ortho2 = torch.mean(torch.sum(specific_2d_norm * shared_2d_norm, dim=-1) ** 2)
            
            # specific 3d与shared 3d正交
            ortho3 = torch.mean(torch.sum(specific_3d_norm * shared_3d_norm, dim=-1) ** 2)
            
            # 拉近损失：最大化余弦相似度（最小化负余弦相似度）
            cosine_sim = torch.sum(shared_2d_norm * shared_3d_norm, dim=-1)
            pull_loss =  torch.mean(1 - cosine_sim) # 或者使用 torch.mean(1 - cosine_sim)
            
            # 累加单个样本loss
            total_loss += (ortho1 + ortho2 + ortho3 + pull_loss)
    ...
    def forward(self, batch_data):
        ...
        # fusion
        molecule_embeddings = self.fusion0(torch.cat([embeddings_2D, embeddings_3D], dim=-1))  # (195, 128)
        ...
        for feature_key in self.feature_visit_evnet_keys:
            if feature_key != 'drugs_hist':
                ...
                drug_emb_2d_shared_list = []          # 新增shared 2d存储列表
                drug_emb_3d_shared_list = []          # 新增shared 3d存储列表
                drug_emb_2d_specific_list = []        # 新增specific 2d存储列表
                drug_emb_3d_specific_list = []        # 新增specific 3d存储列表
                ...
                for i in range(batch_size):
                    if len(drugs_hist[i]) < 2:  # 如果没有药物历史，跳过
                        ...
                        drug_emb_2d_shared_list.append(torch.zeros(1, self.embedding_dim, device=self.device))
                        drug_emb_3d_shared_list.append(torch.zeros(1, self.embedding_dim, device=self.device))
                        drug_emb_2d_specific_list.append(torch.zeros(1, self.embedding_dim, device=self.device))
                        drug_emb_3d_specific_list.append(torch.zeros(1, self.embedding_dim, device=self.device))
                        continue
                    for j in range(len(drugs_hist[i])-1):  #处理每个visit
                        ...
                        # 共享编码器处理
                        shared_2d = self.shared_encoder_2d(drug_emb_2d)
                        shared_3d = self.shared_encoder_3d(drug_emb_3d)
                        
                        # 特定编码器处理
                        specific_2d = self.specific_encoder_2d(drug_emb_2d)
                        specific_3d = self.specific_encoder_3d(drug_emb_3d)
                        
                        # 计算平均嵌入
                        shared_emb = (shared_2d + shared_3d) / 2
                        
                        # 添加到列表
                        drug_emb_2d_shared_list.append(shared_2d)
                        drug_emb_3d_shared_list.append(shared_3d)
                        drug_emb_2d_specific_list.append(specific_2d)
                        drug_emb_3d_specific_list.append(specific_3d)

                        # 拼接融合
                        fusion_input = torch.cat([shared_emb, specific_2d, specific_3d], dim=-1)
                        drug_emb_fused = self.fusion(fusion_input)
        ...
        # 在第778行之前添加调试信息
        # print(f"drug_emb_2d_origin_list shape: {drug_emb_2d_origin_list.shape}")
        # print(f"drug_emb_3d_origin_list shape: {drug_emb_3d_origin_list.shape}")
        # print(f"drug_emb_2d_recon_list shape: {drug_emb_2d_recon_list.shape}")
        # print(f"drug_emb_3d_recon_list shape: {drug_emb_3d_recon_list.shape}")

        # 检查是否有NaN或inf
        # print(f"2D origin has NaN: {torch.isnan(drug_emb_2d_origin_list).any()}")
        # print(f"3D origin has NaN: {torch.isnan(drug_emb_3d_origin_list).any()}")
        # print(f"2D recon has NaN: {torch.isnan(drug_emb_2d_recon_list).any()}")
        # print(f"3D recon has NaN: {torch.isnan(drug_emb_3d_recon_list).any()}")

        loss_recon_2D = contrastive_loss(drug_emb_2d_recon_list, drug_emb_2d_origin_list, temperature=0.07)
        loss_recon_3D = contrastive_loss(drug_emb_3d_recon_list, drug_emb_3d_origin_list, temperature=0.07)
        loss_recon = 0.5 * (loss_recon_2D + loss_recon_3D)

        # 计算shared encoder与specific encoder的损失
        drug_emb_2d_shared_list = torch.cat(drug_emb_2d_shared_list, dim=0)
        drug_emb_3d_shared_list = torch.cat(drug_emb_3d_shared_list, dim=0)
        drug_emb_2d_specific_list = torch.cat(drug_emb_2d_specific_list, dim=0)
        drug_emb_3d_specific_list = torch.cat(drug_emb_3d_specific_list, dim=0)

        loss_orth = self.calculate_orthogonality_loss(drug_emb_2d_shared_list, drug_emb_3d_shared_list, drug_emb_2d_specific_list, drug_emb_3d_specific_list)
        ...
        return logits, loss_recon, loss_orth