...
class MAB_ddi(torch.nn.Module):
    def __init__(
        self, Qdim, Kdim, Vdim, number_heads,
        use_ln=False, *args, **kwargs
    ):
        super(MAB_ddi, self).__init__(*args, **kwargs)
        self.Vdim = Vdim
        self.number_heads = number_heads
    ...
    def forward(self, X, Y, molecule_ddi):
        Q, K, V = self.Qdense(X), self.Kdense(Y), self.Vdense(Y)
        batch_size, dim_split = Q.shape[0], self.Vdim // self.number_heads
        ...
        ddi_matrix_expanded = molecule_ddi.unsqueeze(0).expand_as(Attn)
        ddi_matrix_expanded = ddi_matrix_expanded * -1e9
        Attn = Attn + ddi_matrix_expanded
...
class SAB_ddi(torch.nn.Module):
    def __init__(
        self, in_dim, out_dim, number_heads,
        use_ln=False, *args, **kwargs
    ):
        super(SAB_ddi, self).__init__(*args, **kwargs)
        self.net = MAB_ddi(in_dim, in_dim, out_dim, number_heads, use_ln)

    def forward(self, X, molecule_ddi):
        molecule_ddi = molecule_ddi.to(torch.float32)
        return self.net(X, X, molecule_ddi)